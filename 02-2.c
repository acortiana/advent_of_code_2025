...
int test_string_equality(char *a, char *b, int len) {
    for (int i=0; i < len; i++) {
        if (*(a+i) != *(b+i)) { return 0; }
    }
    return 1;
...
int str_repeated_at_least_twice(char *a) {
    int len = strlen(a);
    int halflen = len / 2;
    for (int i=1; i <= halflen; i++) {
        int result = 1;
        for (int j=0; (i+j) < len; j+=i) {
            if (! test_string_equality(a,a+j+i,i)) { result = 0; break; }
        }
...
unsigned long int process(char *a, char *b) {
    ...
    for (unsigned long int i = start; i <= end; i++) {
        char *tmp = itoa(i);
        if (str_repeated_at_least_twice(tmp)) { invalid_sum += i; }
        free(tmp);
    }
    return invalid_sum;
}
